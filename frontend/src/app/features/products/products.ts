...
type MediaDropZone = 'primary' | 'gallery';
...
export class ProductsComponent implements OnInit {
  ...
  protected activeMediaDropZone: MediaDropZone | null = null;
  ...
  protected onPrimaryImageSelected(event: Event): void {
    ...
    if (file) {
      ...
      this.handlePrimaryImageFile(file);
  ...
  protected onMediaDragOver(event: DragEvent, zone: MediaDropZone): void {

    event.preventDefault();



    if (this.isMediaDropZoneDisabled(zone)) {
    ...
    this.activeMediaDropZone = zone;

  }



  protected onMediaDragLeave(event: DragEvent, zone: MediaDropZone): void {
    ...
    if (this.activeMediaDropZone === zone) {
  ...
  protected onMediaDrop(event: DragEvent, zone: MediaDropZone): void {
    ...
    if (zone === 'primary') {
      ...
      if (files.length > 1) {
        ...
        this.mediaError = 'Drop one file for the primary product image.';
      ...
      this.handlePrimaryImageFile(files[0]);
  ...
  protected isMediaDropZoneDisabled(zone: MediaDropZone): boolean {
    ...
    return (
      ...
      (zone === 'gallery' && this.galleryImages.length >= 5)
...
private handlePrimaryImageFile(file: File): void {

    if (this.isDuplicateMediaFile(file)) {

      this.mediaError = 'This image is already selected in the product media. Remove it before uploading the same file again.';
    ...
    this.uploadMediaFile(file, (uploadedImage) => {
      ...
      uploadedImage.fileSignature = this.getMediaFileSignature(file);
...
private handleGalleryImageFiles(files: File[]): void {

    if (!files.length || this.isMediaDropZoneDisabled('gallery')) {
    ...
    for (const file of files) {
      ...
      const signature = this.getMediaFileSignature(file);



      if (seenSignatures.has(signature) || this.isDuplicateMediaFile(file)) {

        this.mediaError = 'One or more selected images are already in the product media. Remove duplicates before uploading.';
    ...
    if (remainingSlots <= 0) {
      ...
      this.mediaError = 'The gallery already has the maximum of five images.';
...
private uploadGalleryFiles(files: File[], index = 0): void {
  ...
  private getMediaFileSignature(file: File): string {
  ...
  private isDuplicateMediaFile(file: File): boolean {
    ...
    return (
      ...
      this.primaryImage?.fileSignature === signature ||

      this.galleryImages.some((image) => image.fileSignature === signature)
